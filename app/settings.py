...
# Внутренние пути по умолчанию настроим на папку `repo/`, чтобы
# модель могла храниться в Git-отслеживаемой директории `repo/models`
REPO_ROOT = Path(__file__).resolve().parents[1]  # .../gpumassls_task_b10/repo


class Settings(BaseSettings):
    model_config = {"protected_namespaces": ("settings_",)}
    model_dir: str = str(REPO_ROOT / "models/ner_xlmr_wordlevel_entity")
    labels_path: str = str(REPO_ROOT / "models/ner_xlmr_wordlevel_entity/labels.txt")
    max_seq_len: int = 256
