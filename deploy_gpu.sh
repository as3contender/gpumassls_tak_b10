...
# ==== настройка (локальная) ====
USER_ON_HOST="${USER_ON_HOST:-ubuntu}"             # свой юзер на сервере, можно USER_ON_HOST=denis ./deploy_gpu.sh
HOST="89.169.187.74"
...
echo "==> Подготовка сервера $SSH (Docker, compose, ufw, каталоги, NVIDIA drivers)"
...
if ! command -v nvidia-smi >/dev/null 2>&1; then
  echo "Установка NVIDIA drivers и Container Toolkit..."
  # Установка NVIDIA drivers (если нужно)
...
nvidia-smi || echo "NVIDIA GPU не обнаружен или драйверы не установлены"
REMOTE
